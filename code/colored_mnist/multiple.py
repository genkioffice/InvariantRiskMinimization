...
def make_environment(images, labels, p_color):
    ...
    bin_labels = (labels < 5).float()
    bin_labels = torch_xor(bin_labels, torch_bernoulli(0.25, len(bin_labels))) # accurecyのmaxはcolorの情報を使わなければ最大が.75である。

    colors = torch_xor(bin_labels, torch_bernoulli(p_color, len(bin_labels)))
...
for restart in range(flags.n_restarts):
    ...
    envs = [
        make_environment(mnist_train[0][::3], mnist_train[1][::3], 0.2), # 0.8の確率でlabel->colorを
        make_environment(mnist_train[0][1::3], mnist_train[1][1::3], 0.1), # 0.9の確率
        make_environment(mnist_train[0][2::3], mnist_train[1][2::3], 0.3), # 0.7の確率
    ...
    for step in range(flags.steps):
        for env in envs:
            ...
            env['nll'] = loss(logits, env['labels'])
...
print(data)
data.to_pickle("results_multiple.pickle")
